...
async function tornGet(endpoint, selections = '') {
  ...
  try {
    ...
    // ============================================
    // TORN API (V2 PATH STYLE)
    // ============================================

    function buildTornV2Url(path, params = {}, key = tornApiKey) {
      const p = String(path || '').trim();
      if (!p) throw new Error('endpoint path required');
      ...
      return `${CONFIG.torn.baseUrl}/v2${p.startsWith('/') ? '' : '/'}${p}?${qs.toString()}`;
    }

    async function tornV2Get(path, params = {}, keyOverride = null) {
      const key = keyOverride !== null ? keyOverride : tornApiKey;
      if (!key) throw new Error('Torn API key not set');

      const cacheKey = `tornv2:${path}:${JSON.stringify(params || {})}`;
      const cached = getCached(cacheKey, CONFIG.torn.cacheTime);
      if (cached) {
        stats.cacheHits++;
        return cached;
      ...
      const url = buildTornV2Url(path, params, key);
      const t0 = performance.now();
      stats.totalCalls++;
      odinLogApi({ ts: Date.now(), service: 'torn', method: 'GET', url, endpoint: String(path || ''), selections: '', cached: false, ok: false, ms: 0 });
      ...
      setCache(cacheKey, data);
      odinLogApi({ ts: Date.now(), service: 'torn', method: 'GET', url, endpoint: String(path || ''), selections: '', cached: false, ok: true, ms });
      return data;
    }
    ...
    async function validateTornKeyCapabilities(key) {
      ...
      const checks = [
        { id: 'user.basic', label: 'User Basic', path: '/user/basic' },
        { id: 'user.attacks', label: 'User Attacks', path: '/user/attacks' },
        { id: 'faction.basic', label: 'Faction Basic', path: '/faction/basic' },
        { id: 'faction.members', label: 'Faction Members', path: '/faction/members' },
        { id: 'faction.chain', label: 'Faction Chain', path: '/faction/chain' },
        { id: 'faction.rankedwars', label: 'Faction Ranked Wars', path: '/faction/rankedwars' },
      ];

      const results = {};
      for (const c of checks) {
        try {
          await tornV2Get(c.path, {}, trimmedKey);
          results[c.id] = { ok: true, label: c.label };
        } catch (e) {
          results[c.id] = { ok: false, label: c.label, error: (e && e.message) ? e.message : String(e), code: e && e.code };
        }
      }
...
/**
    ...
    async function tornStatsGet(endpoint, version = 'v2') {
      ...
      // Check cache
      const cacheKey = `ts:${version}:${endpoint}`;
      ...
      // TornStats v2 commonly uses the key as a path segment:
      // https://www.tornstats.com/api/v2/<KEY>/spy/user/<ID>
      // Some endpoints also accept ?key=<KEY>. We try path-key first then fallback.
      const urlPathKey = version === 'v2'
        ? `${baseUrl}/${encodeURIComponent(tornStatsApiKey)}${endpoint.startsWith('/') ? '' : '/'}${endpoint}`
        : `${baseUrl}${endpoint}?key=${encodeURIComponent(tornStatsApiKey)}`;

      const urlQueryKey = `${baseUrl}${endpoint}${endpoint.includes('?') ? '&' : '?'}key=${encodeURIComponent(tornStatsApiKey)}`;
      ...
      try {
        ...
        // First attempt
        try {
          const r1 = await tryFetchJson(urlPathKey);
          data = r1.data;
          ...
          // Fallback attempt
          const r2 = await tryFetchJson(urlQueryKey);
          data = r2.data;
          if (data && (data.error || data.status === 'error')) {
            throw new Error(data.message || data.error || 'TornStats API error');
          }
    ...
    async function tornStatsSpyGet(playerId) {
      ...
      // Prefer the documented v2 path format: /spy/user/<id>
      try {
        return await tornStatsGet(`/spy/user/${encodeURIComponent(pid)}`, 'v2');
      } catch (e1) {
        // Backwards compatibility with older endpoint: /spy/<id>
    ...
    async function tornStatsFactionGet(factionId) {
      ...
      if (!fid) throw new Error('factionId required');

      // Prefer the documented v2 path format: /spy/faction/<id>
      try {
        return await tornStatsGet(`/spy/faction/${encodeURIComponent(fid)}`, 'v2');
      } catch (e1) {
        // Backwards compatibility with older endpoint: /faction/<id>
    ...
    function buildTornStatsUrl(endpoint, version = 'v2') {
      const baseUrl = version === 'v1' ? CONFIG.tornStats.baseUrlV1 : CONFIG.tornStats.baseUrlV2;
      return version === 'v2' ? `${baseUrl}/${tornStatsApiKey}${endpoint.startsWith('/') ? '' : '/'}${endpoint}` : `${baseUrl}${endpoint}?key=${tornStatsApiKey}`;
    }
