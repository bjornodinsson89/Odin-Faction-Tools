...
class OdinUserInterface extends BaseModule {
  ...
  updateChainWatcherUI(data) {
    ...
    const hitsToNext = typeof data.hitsToNext === 'number' ? data.hitsToNext : null;
    ...
    if (chainSpan) {
      chainSpan.textContent = `${current} / ${max || 0}`;
    }
    if (timerSpan) {
      timerSpan.textContent = Utils.formatTime(timeout, false);
    }
    if (bonusSpan) {
      if (nextBonus) {
    ...
    if (sessionSpan) {
      sessionSpan.textContent = Utils.formatTime(sessionSeconds, false);
    }
    if (barFill) {
  ...
  async renderContent(container) {
    ...
    html += '<div id="odin-menu">'
      ...
      + '<button class="odin-menu-btn" data-section="spear">Spear</button>'
        ...
        reader.onload = ev => {
          ...
          try {
            ...
            Object.values(imported).forEach(f => {
              if (!f.members || typeof f.members !== 'object' || typeof f.name !== 'string')
...
<div class="table-container">
      ...
      ${rows.map((row, idx) => {
        ...
        return `
          <tr data-id="${id}">
            ...
            <td>
              <button class="odin-spear-claim" data-id="${id}">Claim</button>
              <button class="odin-spear-release" data-id="${id}">Release</button>
              <button class="odin-spear-focus" data-id="${id}">Focus</button>
            </td>
          </tr>`;
      }).join('')}
      ${rows.length === 0 ? `<tr><td colspan="11">No spear rows yet.</td></tr>` : ''}
    </tbody>
  </table>
</div>`;
  }
  ...
  attachSpearListeners(sectionContent) {
    ...
    sectionContent.querySelectorAll('.odin-spear-claim').forEach(btn =>
      btn.addEventListener('click', () => fire('claim', { id: btn.dataset.id }))
    );
    sectionContent.querySelectorAll('.odin-spear-release').forEach(btn =>
      btn.addEventListener('click', () => fire('release', { id: btn.dataset.id }))
    );
    sectionContent.querySelectorAll('.odin-spear-focus').forEach(btn =>
      btn.addEventListener('click', () => {
        fire('focus', { id: btn.dataset.id });
        window.location.href = 'https://www.torn.com/profiles.php?XID=' + btn.dataset.id;
      })
    );
  }
  ...
  onSpearOverlayState(payload) {
    ...
    if (activeBtn && activeBtn.dataset.section === 'spear') {
      const sectionContent = document.querySelector('#odin-section-content');
      if (sectionContent) {
        sectionContent.innerHTML = this.renderSpear();
        this.attachSpearListeners(sectionContent);
...
(async () => {
  ...
  Nexus.on("SPEAR_OVERLAY_STATE", (payload) => {
    if (ui && typeof ui.onSpearOverlayState === "function") {
      ui.onSpearOverlayState(payload || {});
    }
  });
