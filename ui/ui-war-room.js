  ...
  window.OdinModules.push(function OdinUIModule_WarRoomInit(OdinContext) {
    ...
    function renderWarRoom() {
      ...
      if (currentShift) {
        ...
        const watcherCard = UI.createCard(null, `
      ...
      const statsCard = UI.createCard(null, `
    ...
    function init() {
      ...
      _unsubs.push(nexus.on('CLAIM_MADE', () => {
        if (window.OdinUI?.getState()?.activeTab === 'war-room') {
          window.OdinUI.refreshContent();
        }
      }));

      _unsubs.push(nexus.on('CLAIM_RELEASED', () => {
        if (window.OdinUI?.getState()?.activeTab === 'war-room') {
          window.OdinUI.refreshContent();
        }
      }));
