  ...
  window.OdinModules.push(function OdinUIModule_WatchersInit(OdinContext) {
    ...
    function renderWatchers() {
      ...
      if (currentShift) {
        ...
        const currentCard = UI.createCard(null, `
        ...
        const emptyCard = UI.createCard(null, `
      ...
      const addCard = UI.createCard(null, `
    ...
    function init() {
      ...
      _unsubs.push(nexus.on('WATCHER_SHIFT_START', () => {
        if (window.OdinUI?.getState()?.activeTab === 'watchers') {
          window.OdinUI.refreshContent();
        }
      }));

      _unsubs.push(nexus.on('WATCHER_SHIFT_END', () => {
        if (window.OdinUI?.getState()?.activeTab === 'watchers') {
          window.OdinUI.refreshContent();
        }
      }));
