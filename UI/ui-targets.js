...
(function () {
  ...
  window.OdinModules.push(function OdinUIModule_TargetsInit(OdinContext) {
    ...
    function renderTargets() {
      ...
      const medDeals = spear?.MedDealsService?.getActiveDeals?.() || [];

      const claimedIds = new Set(claims.map((c) => c.targetId));
      const dibMap = new Map(dibs.map((d) => [String(d.targetId), d]));
      const dealMap = new Map(medDeals.map((d) => [String(d.targetId), d]));
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              ...
              <tbody>
                ${filteredTargets.map((target, idx) => {
                  ...
                  const dib = dibMap.get(String(target.id)) || null;
                  const deal = dealMap.get(String(target.id)) || null;
                  const isDibbed = !!dib && dib.status === 'active';
                  const isDealt = !!deal && deal.status === 'active';
                  const isMineDib = isDibbed && String(dib.attackerId) === String(ctx.userId || 'unknown');
                  const isMineDeal = isDealt && String(deal.attackerId) === String(ctx.userId || 'unknown');
                    ...
                    ">
                      ...
                      <td style="padding: 10px 12px;">
                        <div style="display: flex; gap: 4px;">
                          ...
                          ${spear?.DibsService ? `
                            ${!isDibbed ? `
                              <button class="target-dib odin-btn odin-btn-secondary" data-id="${target.id}" style="padding: 4px 8px; font-size: 11px;">
                                Dibs
                              </button>
                            ` : isMineDib ? `
                              <button class="target-undib odin-btn odin-btn-secondary" data-id="${target.id}" style="padding: 4px 8px; font-size: 11px;">
                                UnDib
                              </button>
                            ` : `
                              <span style="padding: 4px 8px; font-size: 11px; color: #718096;">
                                Dib: ${dib.attackerName || '?'}
                              </span>
                            `}
                          ` : ''}

                          ${spear?.MedDealsService ? `
                            ${!isDealt ? `
                              <button class="target-meddeal odin-btn odin-btn-secondary" data-id="${target.id}" style="padding: 4px 8px; font-size: 11px;">
                                Med
                              </button>
                            ` : isMineDeal ? `
                              <button class="target-unmeddeal odin-btn odin-btn-secondary" data-id="${target.id}" style="padding: 4px 8px; font-size: 11px;">
                                UnMed
                              </button>
                            ` : `
                              <span style="padding: 4px 8px; font-size: 11px; color: #718096;">
                                Med: ${deal.attackerName || '?'}
                              </span>
                            `}
                          ` : ''}
...
// ============================================
    ...
    function attachEventListeners() {
      ...
      document.querySelectorAll('.target-dib').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          const targetId = e.target.dataset.id;
          const result = spear?.DibsService?.makeDib?.(
            targetId,
            ctx.userId || 'unknown',
            ctx.userName || 'You'
          );
          if (result?.success) {
            ...
            alert(`Dibs already claimed by ${result.claimedBy}`);
          } else if (result?.reason === 'claimer_traveling') {
            alert('You cannot dibs while traveling/abroad');
          } else if (result?.reason === 'disabled') {
            alert('Dibs are disabled (or attack mode is not FARMING)');
          } else {
            alert('Failed to claim dibs');
          }
        });
      });

      document.querySelectorAll('.target-undib').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          const targetId = e.target.dataset.id;
          spear?.DibsService?.releaseDib?.(targetId, ctx.userId || 'unknown');
          window.OdinUI?.refreshContent();
        });
      });

      document.querySelectorAll('.target-meddeal').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          const targetId = e.target.dataset.id;
          const note = prompt('Med deal note (optional):') || '';
          const result = spear?.MedDealsService?.makeDeal?.(
            targetId,
            ctx.userId || 'unknown',
            ctx.userName || 'You',
            note
          ...
          if (result?.success) {
            ...
            alert(`Med deal already claimed by ${result.claimedBy}`);
          } else if (result?.reason === 'disabled') {
            alert('Med deals are disabled (or attack mode is not FARMING)');
          } else {
            alert('Failed to claim med deal');
          }
        });
      });

      document.querySelectorAll('.target-unmeddeal').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          const targetId = e.target.dataset.id;
          spear?.MedDealsService?.releaseDeal?.(targetId, ctx.userId || 'unknown');
          window.OdinUI?.refreshContent();
        });
      });
