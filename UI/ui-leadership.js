...
(function () {
  ...
  window.OdinModules.push(function OdinUIModule_LeadershipInit(OdinContext) {
    ...
    function renderLeadership() {
      ...
      const configCard = UI.createCard(`
        <div style="display: flex; flex-direction: column; gap: 16px;">
          ...
          <div>
            ...
            ">
              ...
              <option value="ranked" ${warConfig.warType === 'ranked' ? 'selected' : ''}>Ranked War (RW)</option>
              <option value="territory" ${warConfig.warType === 'territory' ? 'selected' : ''}>Territory War (TW)</option>
            </select>
          </div>
        ...
        if (factionId) {
          try {
            // Try to fetch faction name from Torn
                        const api = OdinContext?.api || ctx.api || window.OdinContext?.api;
