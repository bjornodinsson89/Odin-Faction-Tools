...
(function () {
  ...
  window.OdinModules.push(function OdinUIModule_SettingsInit(OdinContext) {
    ...
    function renderSettings() {
      ...
      // ============================================
      // WAR & COORDINATION SECTION
      // ============================================
      const warSection = UI.createSection('War & Coordination', '⚔️');
      const spear = window.OdinSpear || window.OdinContext?.spear || window.OdinContext?.services || {};
      const warCfg = spear?.WarConfigService?.getConfig?.() || {};
      const attackMode = warCfg.attackMode || 'FARMING';
      const dibsEnabled = warCfg.dibs?.enabled !== false;
      const medEnabled = warCfg.medDeals?.enabled !== false;
      const spiesEnabled = warCfg.spies?.enabled === true;
      const liveHooksEnabled = warCfg.presence?.enableLiveHooks !== false;

      const warCard = UI.createCard(`
        <div style="display: flex; flex-direction: column; gap: 12px;">
          <div>
            <label style="color: #a0aec0; font-size: 12px;">Attack Mode</label>
            <select id="setting-attack-mode" style="width: 100%; padding: 8px; margin-top: 6px; border-radius: 6px; background: rgba(26, 32, 44, 0.8); border: 1px solid rgba(74, 85, 104, 0.6); color: #e2e8f0;">
              <option value="FARMING" ${attackMode === 'FARMING' ? 'selected' : ''}>FARMING (normal coordination)</option>
              <option value="FFA" ${attackMode === 'FFA' ? 'selected' : ''}>FFA (free-for-all)</option>
              <option value="TURTLE" ${attackMode === 'TURTLE' ? 'selected' : ''}>TURTLE (defensive)</option>
            </select>
            <div style="color: #718096; font-size: 11px; margin-top: 6px;">
              When not FARMING, Odin will automatically disable claims/dibs/med deals.
            </div>
          </div>

          <div style="display: flex; flex-direction: column; gap: 8px;">
            <label style="display: flex; align-items: center; gap: 8px; color: #e2e8f0; font-size: 14px;">
              <input type="checkbox" id="setting-dibs-enabled" ${dibsEnabled ? 'checked' : ''} style="width: 16px; height: 16px;">
              Enable Dibs
            </label>

            <label style="display: flex; align-items: center; gap: 8px; color: #e2e8f0; font-size: 14px;">
              <input type="checkbox" id="setting-med-enabled" ${medEnabled ? 'checked' : ''} style="width: 16px; height: 16px;">
              Enable Med Deals
            </label>

            <label style="display: flex; align-items: center; gap: 8px; color: #e2e8f0; font-size: 14px;">
              <input type="checkbox" id="setting-spies-enabled" ${spiesEnabled ? 'checked' : ''} style="width: 16px; height: 16px;">
              Enable TornStats Spies (needs TornStats key)
            </label>

            <label style="display: flex; align-items: center; gap: 8px; color: #e2e8f0; font-size: 14px;">
              <input type="checkbox" id="setting-live-hooks" ${liveHooksEnabled ? 'checked' : ''} style="width: 16px; height: 16px;">
              Enable Live War Presence Hooks (fetch/WebSocket)
            </label>
          </div>
        </div>
      `);

      warSection.appendChild(warCard);
  ...
  try {
    ...
    if (statsEl) statsEl.textContent = '';
      ...
      document.getElementById('setting-save')?.addEventListener('click', () => {
        ...
        try {
          const spear = window.OdinSpear || window.OdinContext?.spear || window.OdinContext?.services || {};
          const warSvc = spear?.WarConfigService;
          if (warSvc && typeof warSvc.getConfig === 'function' && typeof warSvc.updateConfig === 'function') {
            const current = warSvc.getConfig() || {};
            const attackMode = document.getElementById('setting-attack-mode')?.value || current.attackMode || 'FARMING';
            const dibsEnabled = !!document.getElementById('setting-dibs-enabled')?.checked;
            const medEnabled = !!document.getElementById('setting-med-enabled')?.checked;
            const spiesEnabled = !!document.getElementById('setting-spies-enabled')?.checked;
            const liveHooksEnabled = !!document.getElementById('setting-live-hooks')?.checked;

            warSvc.updateConfig({
              attackMode,
              dibs: { ...(current.dibs || {}), enabled: dibsEnabled },
              medDeals: { ...(current.medDeals || {}), enabled: medEnabled },
              spies: { ...(current.spies || {}), enabled: spiesEnabled },
              presence: { ...(current.presence || {}), enableLiveHooks: liveHooksEnabled },
            });
          }
        } catch (e) {}
