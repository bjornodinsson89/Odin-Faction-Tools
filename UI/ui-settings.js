...
(function () {
  ...
  window.OdinModules.push(function OdinUIModule_SettingsInit(OdinContext) {
    ...
    function attachEventListeners() {
      ...
      const refreshDiag = () => {
        try {
          const lines = window.OdinDiag?.getLog ? window.OdinDiag.getLog() : [];
          if (ta) ta.value = (lines || []).join('\n');
          const apiStats = (ctx.api && typeof ctx.api.getStats === 'function') ? ctx.api.getStats() : null;
          if (statsEl) {
            const s = apiStats || {};
            const calls = (s && typeof s.totalCalls === 'number') ? s.totalCalls : 0;
            const ok = (s && typeof s.okCalls === 'number') ? s.okCalls : 0;
            const fail = (s && typeof s.failCalls === 'number') ? s.failCalls : 0;
            const cache = (s && typeof s.cacheHits === 'number') ? s.cacheHits : 0;
            statsEl.textContent = `Torn API: ${calls} calls (${ok} ok, ${fail} fail) • Cache hits: ${cache}`;
          }
        } catch (e) {}
      };
